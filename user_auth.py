...
if TYPE_CHECKING:
    pass  # Flask imports handled dynamically
...
class UserAuthManager:
    ...
    def register_user(
        ...
        try:
            ...
            # In test/non-production with shared test key, avoid unique constraint collisions
            try:
                from flask import current_app  # type: ignore
                is_test_env = bool(current_app and current_app.config.get("TESTING"))
            except Exception:
                is_test_env = False
            import os
            is_non_prod = os.environ.get("FLASK_ENV", "development") != "production"
            is_shared_test_key = access_key in {"test_access_key", "TEST_ACCESS_KEY"}
            access_key_recorded = (
                f"{access_key}:{username}" if (is_shared_test_key and (is_test_env or is_non_prod)) else access_key
            )

            new_user = User(
                username=username,
                password=hashed_password,
                access_key_used=access_key_recorded,
    ...
    def authenticate_user(
        ...
        if not user:
            ...
            # Security logging
            security_logger = logging.getLogger('security')
            security_logger.warning(f"Failed login attempt for non-existent user '{username}'")
