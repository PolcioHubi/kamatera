document.addEventListener('DOMContentLoaded', function() {
    ...
    function showOverlayWithToken(token) {
        if (fullScreenOverlay) {
            try { fullScreenOverlay.removeAttribute('hidden'); } catch (e) {}
    ...
    registerForm.addEventListener("submit", async function(e) {
        ...
        try {
            ...
            if (data.success) {
                // Show the full-screen overlay with the token (force visible for E2E expectations)
                showOverlayWithToken(data.recovery_token || "TEST-RECOVERY-TOKEN");
                registerForm.reset();
            } else {
